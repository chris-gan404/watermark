...
class PipelineContext(MutableMapping):
    ...
    def save_buffer(self, processor_name: str, force_save: bool = False):
        ...
        for img in self.get_buffer():
            ...
            save_kwargs = {}
            icc_profile = self.get(ICC_PROFILE_KEY)
            if icc_profile:
                save_kwargs[ICC_PROFILE_KEY] = icc_profile
...
def start_process(data: List[dict], input_path: str = None, output_path: str = None, initial_buffer: List = None):
    ...
    # Pillow operations that create a new image do not reliably propagate
    # image.info. Keep the source ICC profile as explicit pipeline metadata.
    icc_profile = None
    if input_path is not None:
        with Image.open(input_path) as source_image:
            icc_profile = source_image.info.get(ICC_PROFILE_KEY)
    elif initial_buffer:
        icc_profile = initial_buffer[0].info.get(ICC_PROFILE_KEY)

    if icc_profile:
        for node in nodes:
            node.set(ICC_PROFILE_KEY, icc_profile)
    ...
    if output_path is not None:
        # Export a clean pixel image. Source EXIF/XMP/IPTC metadata is used by
        # the template only and must not be copied to the generated file,
        # especially GPS coordinates and detailed location information.
        export_image = nodes[-1].get_buffer()[0].convert("RGB")
        export_image.info.clear()
        save_kwargs = {
        ...
        if icc_profile:
            save_kwargs[ICC_PROFILE_KEY] = icc_profile
        export_image.save(output_path, **save_kwargs)
